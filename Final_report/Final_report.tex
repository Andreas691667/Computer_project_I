...
\section{Design and implementation}
    ...
    \end{figure}
    We thus only considered four distinct cases for our intitial setup. However, we later found that this was not sufficient. Our solution to this will be elaborated in the next subsection. 
...
We will elaborate this further in subsection \ref{subsec:arduino_as_blocks}. 
    ...
    \end{enumerate}
    In practice we only needed to run three commands when the robot should be tested.
    ...
    Then, in a different terminal we should run the code, we had written ourselves by writing the command: \texttt{roslaunch turtlebot3\_example\ turtlebot3\_obstacle.launch}.
    ...
    We were given a robot navigation example from a GitHub repository\footnote{The Github Repository of the first navigation example is linked in the references} from where we were to get inspiration and further implement our own robot navigation. The idea of the initial example was to: \emph{Drive forward until we get close to an obstacle, then stop}. The robot kept driving by initiating the following \texttt{while}-loop
...
\end{lstlisting} \end{minipage}
    ...
    \begin{enumerate}
        ...
        \item Here, we update the twist command with respect to the angular velocity which in this case should be zero since the robot should \emph{only} turn backwards. 
        ...
        \item We write to log what we are doing. 
        \item We wanted the robo to turn backwards for some time, in this case 1 second. 
        \item Now the robot should only turn and we thus set the linear speed to zero.
...
\end{lstlisting} \end{minipage} 
    ...
    \subsubsection{Tuples in python} When testing the robot we encountered that the LiDAR would return a lot of zeros which would represent an obstacle too close to the robot such that the robot would and should react to. This is because the LiDAR sensor has a max range of \(3.5\) m, so if an obstacle was further away than this it would just set the distance data to \(0\) which was a problem. We solved this by converting the zeros to \(3.5\) (max distance) instead, but in Python you can not edit data in tuples, so we had to convert this list of tuples to a list of lists and from there convert the zeros to \(3.5\). We created the following function \texttt{non\_zeros} to do so.
...
\begin{enumerate}
    \item Since we wanted the highest possible linear velocity, we set the default value of \texttt{LINEAR\_VEL} to \(0.22 \frac{m}{s}\), which is the maximum linear velocity of the turtlebot according to its documentation. 
    ...
    \item This is the maximal angular velocity measured in \(\frac{rad}{s}\) according to the specifications of the turtlebot. 
...
\end{enumerate}
    ...
    We thus empirically added a factor of \(0.08\)cm to the \texttt{SAFE\_STOP\_DISTANCE} constant in this case. By doing this, the robot began avoiding the obstacle earlier, 
...
\end{lstlisting} \end{minipage} 
    ...
    We chose this value since the minimum distance the LiDAR sensor can measure is \(0.12\)m. We did not want to get below this value since it could result in a faulty reading.\footnote{We would not actually get below this value with the true error of the sensor. See footnote \ref{footnote:lidar_accuracy}.}.
...
def emergency_check(l):
    ...
    Instead, the robot should turn \emph{in place} with maximal angular speed. Note that we initially wanted the turtlebot to drive backwards and then turn, but while testing the implementation from listing \ref{lst:initial_case_1} we noticed that backing out was not an option unless we furthered our vision span to also look behind, since we could risk backing into an obstacle as illustrated below.
    \begin{figure}[H]
        \centering
...
Again, we have divided it into the four main parts of the project described earlier. 
    ...
    \begin{enumerate}
        ...
        Given the amount of time of time allocated to this project, we decided to focus on the more important aspects, which would be the programming and optimizations of the robot. \\
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        In turned out that we had used it incorretly and the indexing was actually opposite of what we thought. 
        ...
        We the choice of \(120\) degrees, we had a complete view of the field of view of the robot and achieved the following:
        \begin{itemize}
    ...
    \begin{enumerate}
        ...
        It should, however, not slow down the robot too much since we still wanted a high average linear speed. Thus we chose power regression for linear velocity (the green curve in figure \ref{subfig:regression_types_linear}).  
        \begin{figure}[H]
            \centering
            \begin{subfigure}{.5\textwidth}
              \centering
              \includegraphics[width=.6\linewidth]{regression_types_linear.png}
              \caption{Linear velocity}
              \label{subfig:regression_types_linear}
            \end{subfigure}
            \begin{subfigure}{.5\textwidth}
              \centering
              \includegraphics[width=.6\linewidth]{regression_types_angular.png}
        ...
        Thus we chose power regression for the angular velocity as well. \\
        ...
        It is important to consider the trade-off between linear speed, obstacle avoidance, and areal covered\footnote{Our motivation for the project was namely to simulate a search and rescue robot, which could both avoid obstacles \emph{and} find victims. For this reason it is very important to cover as much area as possible. The 'rescue' part was discarded during the course due to different reasons.}. 
        ...
        We tested the robot with a varíous of different scenarios. Illustrated below are some of the many test-courses on which we tested the final implementation on.
    \end{enumerate}
    \begin{figure}[H]
        \centering
        \includegraphics[width=.7\linewidth]{IMG_3307.jpg}
        \caption{Navigating through tight spaces}
        \label{subfig:tight_spaces}
    \end{figure}
    We encountered that the turtlebot would mostly navigate easily through tight spaces. There were some few occasions where the wheels would graze an obstacle. Since there are obstacles in almost every direction these were the type of test-courses we found to be the most difficult with the most obstacle collisions. We also made the most break throughs in optimizing the navigation when testing through tight spaces. It was here we implemented the \texttt{emergency\_check} function which reduced the amounts of collisions dramatically.
    \begin{figure}[H]
        \centering
        \includegraphics[width=.7\linewidth]{IMG_3308.jpg}
        \caption{Placing obstacles in random places}
        \label{subfig:random_placed_obstacles}
    \end{figure}
    The final implementation navigated through this type of test-course smoothly and without any collisions. It was through these types of test-courses where the optimization of keeping the linear speed as high as possible with regressional input to the \texttt{small\_turns} function was implemented.
    ...
    We have also learned that in order to program a robot, it is crucial to be structured and work systematically with the different aspects this ecompasses. The \emph{Robot operating system} provides this structure, which
    ...
    We did also lose a classmate in the middle of the project, which meant we were only 2 left.
