...
\end{lstlisting} \end{minipage} 
    ...
    The function takes three arguments; a direction, an angular velocity, and a linear velocity which are all used to control the robot\footnote{For our initial implementation we only used the two first inputs and the linear velocity was consistenly equal to \texttt{LINEAR\_VEL}.}. 
...
\end{lstlisting} \end{minipage} 
    Lastly, by default, we drive forward if there are no obstacles to avoid.
...
\end{lstlisting} \end{minipage} 
    Thus, ending an iteration of the control-loop \texttt{while not rospy.is\_shutdown():}.
        ...
        \subsubsection{Retrieving data from the LiDAR sensor}
            We were given a function from the first navigation example to get inspiration from where we implemented our own function:
...
\begin{minipage}{\linewidth} \begin{lstlisting}[style = my_python, caption = Non-zero function, basicstyle = \footnotesize, label = lst:non_zeros]
