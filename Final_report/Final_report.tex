...
\begin{abstract}
    With the continuos improvement of autonomous robots they could sooner than later provide a competetive advantage to more complex tasks such as driving a car or performing a search and rescue operation by reducing error rate, decreasing long-term costs and increasing access to dangerous locations. In this paper, we report on the first hands-on experience with embedded programming, working with virtual machines and ROS and design of real time control software for Aarhus University's computer engineering students. We briefly look into basic arduino programming and use this knowledge to further implement a program for the Turtlebot3 Burger Robot to drive and avoid obstacles autonomously. We conclude ...
\end{abstract}
...
include several code snippets and figures. 
    ...
    \begin{minipage}{\linewidth} \begin{lstlisting}[style = my_python, caption = Initialization of variables]        
...
\end{figure}
    ...
    From there we were to implement our own navigation program such that the robot would avoid the obstacles by changing directions and driving by instead of just stopping. For this we initially drew the four basic cases as described in subsection \ref{subsec: Programming the robot to avoid obstacles}. \\
...
\end{lstlisting} \end{minipage} 
    Lastly, for case \(3\) and \(4\): If an obstacle to the left is within the \texttt{SAFE\_STOP\_DISTANCE} turn right and vice versa.
...
Again, we have divided it into the four main parts of the project described earlier. 
    ...
    \begin{enumerate}
        ...
        % SKRIV OM HVORFOR MEAN I STEDET FOR MIN.
