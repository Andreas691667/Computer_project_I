...
We will elaborate this further in subsection \ref{subsec:arduino_as_blocks}. 
    ...
    Then, in a different terminal we should run the code (node), we had written ourselves by writing the command: \texttt{roslaunch turtlebot3\_example\ turtlebot3\_obstacle.launch}.
...
\end{lstlisting} \end{minipage}
    ...
    \end{enumerate}
    These explanations also hold for all other cases which will thus only be described shortly below. 
...
Again, we have divided it into the four main parts of the project described earlier. 
    ...
    \begin{enumerate}
        ...
        In turned out that we had used it incorretly and the indexing was actually opposite of what we had thought. 
    ...
    \begin{enumerate}
        ...
        It should, however, not slow down the robot too much (as the exponential fit does) since we still wanted a high average linear speed. Thus we chose power regression for linear velocity (the green curve in figure \ref{subfig:regression_types_linear}).  
        \begin{figure}[H]
            \centering
