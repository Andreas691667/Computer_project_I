...
class Obstacle():
    ...
    def get_scan(self):
        ...
        if samples_view is 1:
            ...
            front_lidar_samples1 = scan.ranges[0:front_lidar_samples_ranges+7]
            front_lidar_samples2 = scan.ranges[right_lidar_samples_ranges-7:360]
   ...
   #         elif math.isnan(scan_filter[i]):
    ...
    def obstacle(self):
        ...
        def make_180():
            ...
            twist.angular.z = 0.0
            self._cmd_pub.publish(twist)
            rospy.loginfo('Driving backwards for 1 second')
            t.sleep(1.0)
            twist.linear.x = 0.0
        ...
        while not rospy.is_shutdown():
            ...
            if(front_distance == 0.0):
                front_distance = 3.5
        #	i = len(lidar_distances)
        #	print(i)
        #	print(j)

            # convert to list
            for i in range(len(lidar_distances)):
                lidar_distances[i] = list(lidar_distances[i])

            # if element zero, set to 3.5
            for i in range(len(lidar_distances)):
                for j in range(len(lidar_distances[i])):
                    if lidar_distances[i][j] == 0:
                        lidar_distances[i][j] = 3.5
...
#							small_turns(0)
#							break
#			elif(lidar_distances[1][0] < SAFE_STOP_DISTANCE and lidar_distances[2][-1] < SAFE_STOP_DISTANCE):
#				make_180()
#			elif(front_left_distance < SAFE_STOP_DISTANCE-0.05 and front_right_distance < SAFE_STOP_DISTANCE-0.05):
#				make_180()
#				print('special case')
            elif(front_left_distance < SAFE_STOP_DISTANCE+0.15 or front_right_distance < SAFE_STOP_DISTANCE+0.15):
                if turtlebot_moving:
                    #					if (front_left_distance < SAFE_STOP_DISTANCE and front_right_distance < SAFE_STOP_DISTANCE):
                    #						print('special case')
                    #						make_180()

                    if (front_left_distance < SAFE_STOP_DISTANCE + 0.15):
                        print('blink vinkel venstre')
                        small_turns('right', turn_angle/2, LINEAR_VEL)

                    else:
                        print('blind vinkel hojre')
                        small_turns('left', turn_angle/2, LINEAR_VEL)

            if (front_distance < SAFE_STOP_DISTANCE + 0.08):
                print('obstacle on front')
                if turtlebot_moving:
                    if (right_distance < left_distance):
                        small_turns('left', turn_angle + 0.4, 0.1)
                    elif(left_distance < right_distance):
                        small_turns('right', turn_angle + 0.4, 0.1)

            if(left_distance < SAFE_STOP_DISTANCE or right_distance < SAFE_STOP_DISTANCE):
                if turtlebot_moving:
                    if(right_distance < left_distance):
                        small_turns('left', turn_angle, 0.1)
                    else:
                        small_turns('right', turn_angle, 0.1)


#			elif (lidar_distances[1][0] < 0.25 and lidar_distances[2][-1] < 0.25):
#				make_180()
