...
\section{Introduction}
    ...
    \begin{enumerate}
        \item Understanding the basic concepts of robotics programming by working with an Arduino - that is, the \emph{See-think-act cycle}. 
...
\begin{lstlisting}[style = my_python, caption = \texttt{else-if} statements for Arduino, basicstyle = \footnotesize]
...
\end{lstlisting}
    ...
    \subsubsection{Making sure to avoid the obstacle before next iteration}\label{subsec:avoid_obstacle}
    We wanted to make sure that the robot actualy avoided the obstacle before the next control loop iteration.
    We did this by adding a \texttt{while}-loop to each case:
...
elif(front\_left\_distance < SAFE_STOP_DISTANCE+0.15 or front_right_distance < SAFE_STOP_DISTANCE+0.15):
...
if (front_distance < SAFE_STOP_DISTANCE + 0.08):
    ...
    By doing this we ensured that the robot would not stop avoiding the obstacle before it had actually escaped it.
    Note that the 2nd and 3rd argument in the \texttt{small\_turns} function are written in pseudo-code since we did further optimizations, which will be described in subsection \ref{subsec:optimize_linear_speed}. 
    ...
    These were meant to make the robot able to avoid an obstacle if it was only slightly in front of it, but not so much that the average would get low enough. 
    ...
    We wanted the robot to begin avoiding the obstacles at the blind angles from a distance of \(40\)cm. We got the result as depicted in figure \ref{fig:blind_angle}. 
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{blind_angle.png}
...
scan_filter.append(front_right_lidar_samples) \end{lstlisting}
    ...
    For our initial implementation, our robot would turn with a constant speed of \(0.1 \frac{m}{s}\). Our goal was to achieve an overall higher linear speed.
    ...
    \begin{itemize}
        \item The linear speed should be smaller, the closer the obstacle was.
        \item The angular speed should be greater, the closer the obstacle was.
    \end{itemize}
    ...
    \(x\) is the distance to the obstacle in m.        
        \begin{figure}
            \centering
            \includegraphics[width=0.3\textwidth]{ang_vel_graps.png}
    ...
    \begin{lstlisting}[style = my_python, caption = Power functions for blind spot, basicstyle = \footnotesize, label = lst:blind_spot]
        small_turns('right', 0.34*front_left_distance **
        (-0.93), 1.7*front_left_distance**2.23)
    \end{lstlisting}
    After implementing these changes in the code, we significantly improved the performance of the robot's linear speed.
...
def emergency_check(l):
    ...
    We called this function for each loop iteration, both in the overall control loop as well as in the loops described in subsection \ref{subsec:avoid_obstacle} and depicted in listing \ref{lst:while_loop}.
