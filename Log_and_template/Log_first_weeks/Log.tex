...
\section{Week 9}
    \subsection{First navigation example}
        Through today's slides we setup the workspace and began a basic navigation example that made the turtlebot go forwards until it met an obstacle at a min\_distance then it would stop.

    \subsection{Edit navigation example}
        After getting the first example to work, our job was to edit and play with the code so that it would turn left, right or back up depending on which angle we would meet our obstacle. Just like we did in week 3 with the print statements.\\
    ...
    \subsection{For next week}
        \begin{enumerate}
            \item We are not quite certain if our if-else statements are 100\% correct.
            \item We can not get the turtlebot to go backwards every time we want it to.
            \item We are not quite certain that the indexes of our distances matched their corresponding angles. \\
